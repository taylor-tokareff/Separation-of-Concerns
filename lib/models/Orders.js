...
class Order {
  ...
  static async update(quantityOfItems) {
    const { rows } = await pool.query(
      'UPDATE INTO orders (quantity_of_items) VALUES ($1) RETURNING *',
      [quantityOfItems]
    );
    return rows;
  }

  static async grab(quantityOfItems) {
    const { rows } = await pool.query(
      'SELECT id, quantity_of_items from orders WHERE id = $1', [quantityOfItems]
    );
    return rows;
  }
